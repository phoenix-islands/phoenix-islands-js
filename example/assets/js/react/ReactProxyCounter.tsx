...
export const ReactProxyCounter = ({
  store,
  proxyStore,
  children,
  pushEvent,
}: ReactIslandProps<{ counter: number }>) => {
  const data = useStore(proxyStore);
  const privateData = useStore(store);
  ...
  return (
    <div className="w-full flex flex-col gap-3 items-stretch p-4 rounded-lg border-dashed border-zinc-500 border-2 overflow-hidden relative">
      <div className="absolute top-0 left-0 px-1.5 py-0.5 text-white bg-zinc-500 font-semibold text-xs rounded-br">
        REACT ISLAND
      </div>
      <div className="flex flex-row gap-3 items-center justify-between">
        <div>Private State: {privateData.counter}</div>
        <div>Proxy State: {data.counterData.counter}</div>
